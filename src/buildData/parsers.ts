...
const paramDamageTypeToDamageNumber: Record<ParamDamageType, number> = {
  Physics: AttackPowerType.PHYSICAL,
  Magic: AttackPowerType.MAGIC,
  Fire: AttackPowerType.FIRE,
  Thunder: AttackPowerType.LIGHTNING,
  Dark: AttackPowerType.HOLY,
};
...
const attributeToShorthand: Record<ParamDamageType, DamageAttribute> = {
  Physics: "str",
  Magic: "dex",
  Fire: "int",
  Thunder: "fai",
...
export function parseAshOfWar({
  ...
  const negPhysScale = 0; // TODO: Implement this negscale
  const ineffectiveAttributePenalty = -0.4;
  const equipParamWeaponValue = equipParamWeapons.get(weapon.id);
  ...
  const baseAtkRate = 1 + 3 * (getNormalizedUpgradeLevel(weapon, weaponLevel) / 25); // Also reinforceParamWeapon.baseAtkRate
  const throwFlag = atkParamPcValue.throwFlag;
  const throwAtkRate = equipParamWeaponValue.throwAtkRate;
  // Scaling
  // dex {0: 0.2714727682239568, total: 0.2714727682239568}
  // str {0: 0.14617764442828438, total: 0.14617764442828438}
  // Damage
  // base {0: 181.3, total: 181.3}
  // dex {0: 49.21801287900337, total: 49.21801287900337}
  // str {0: 26.50200693484796, total: 26.50200693484796}
  // The values required to calculate damage.
  return {
...
export function getAshOfWarDamage({
  ...
  const adjustedAttributes: Record<AttackElementCorrectParamAttribute, number> = {
    Strength: adjustStrengthForTwoHanding({ twoHanding, weapon, str: attributes.str }),
    Dexterity: attributes.dex,
    Faith: attributes.fai,
    Luck: attributes.arc,
    Magic: attributes.int,
  };
  // TODO: remove
  // weaponLevel
  // weapon,
  ...
  return paramDamageTypes.reduce((acc, damageType) => {
    ...
    if (ignoreScale) return acc.set(damageType, baseAtk);
    const _baseAtkScaling = damageAttributes.reduce((acc, attr) => {
      // _baseAtkScaling = { Strength => 0.14, Dexterity => 0.29 }
      ...
      if (!isCorrect_by) return acc.set(attr, 0);
      ...
      // TODO: just tried this and didn't work
      const scaling =
        0.01 *
        ...
        changePoint * 0.01;
      const correction = correct_by * 0.01 * _correctRate;
      const damageForAttribute = _correctRate * 0.01 - 1 + scaling * correction;

      return acc.set(attr, damageForAttribute);
    }, new Map());

    const effectiveScaling = isPhysAtkPenalty
      ? ineffectiveAttributePenalty
      : isNegPhysScale
        ? negPhysScale
